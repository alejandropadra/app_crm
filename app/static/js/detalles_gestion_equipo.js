document.addEventListener('DOMContentLoaded', function () {
    ...
    const fichaGetInput = document.getElementById('ficha_get');
    const ficha_get = fichaGetInput ? fichaGetInput.value : null;

    let csrfTokenMeta = document.querySelector('meta[name="csrf-token"]');
    let csrfToken = csrfTokenMeta ? csrfTokenMeta.getAttribute("content") : null;

    // ============================================================
    // BOTÓN EDITAR INDICADORES (solo existe si el usuario tiene permiso)
    // ============================================================
    if (botonEditar && ficha_get) {
        const rutaDestino = `/app_crm/gdd/gestion_equipo_detalles/${ficha_get}`;
    ...
    // ============================================================
    // RETROALIMENTACIÓN (bloque completo depende de elementos
    // que solo existen en ciertos estados/roles)
    // ============================================================
    ...
    const comentarioColabInput = document.getElementById('comentario_colaborador_resultado');
    const comentarioSupervInput = document.getElementById('comentario_superv_resultado');
    const estadoEvaluacionInput = document.getElementById('estado_evaluacion');

    const comentario_colaborador_resultado = comentarioColabInput ? comentarioColabInput.value : "";
    const comentario_superv_resultado = comentarioSupervInput ? comentarioSupervInput.value : "";
    const estado_evaluacion = estadoEvaluacionInput ? estadoEvaluacionInput.value.trim() : "";

    if (estado_evaluacion === "supervisorEvaluacion") {

        if (ComentarioColab) ComentarioColab.readOnly = true;
        ...
        if (comentario_superv_resultado !== "None" && comentario_superv_resultado !== "") {
            if (ComentarioSuperv) ComentarioSuperv.readOnly = true;

            if (enviarRetroalimentacion) {
    ...
    if (enviarRetroalimentacion) {
        enviarRetroalimentacion.addEventListener("click", () => {
            ...
            if (esSupervisor) {
                ...
                if (!ComentarioColab || !ComentarioColab.value.trim()) {
